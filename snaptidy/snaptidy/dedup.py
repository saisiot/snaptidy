...
def find_similar_images(
    image_files: List[str], sensitivity: float, threads: int
) -> List[DuplicateGroup]:
    """
    ...
    valid_image_hashes = [
        (path, data) for path, data in image_hashes if data[0] is not None
    ]
...
def find_similar_videos(
    video_files: List[str], sensitivity: float, threads: int
) -> List[DuplicateGroup]:
    """
    Find similar videos.
    ...
    for i, file_path1 in enumerate(video_files):
        ...
        def compare_video(file_path2: str) -> Tuple[str, bool]:
            ...
            return file_path2, utils.compare_video_frames(
                file_path1, file_path2, sensitivity
            )
        ...
        if videos_to_compare:
            comparison_results = utils.process_in_parallel(
                videos_to_compare, compare_video, threads
            )
...
def remove_duplicates(
    duplicate_groups: List[DuplicateGroup], dry_run: bool = False
) -> Tuple[int, int]:
    """
    ...
    for group in duplicate_groups:
        for duplicate in group.duplicates:
            ...
            logger.info(
                f"{'Would remove' if dry_run else 'Removing'} duplicate: {duplicate}"
            )
...
def run(
    ...
    unprocessed_images = [
        f for f in files_by_type["images"] if f not in processed_files
    ]
    unprocessed_videos = [
        f for f in files_by_type["videos"] if f not in processed_files
    ]

    # Step 2: Find similar images based on perceptual hash
    if unprocessed_images:
        similar_image_groups = find_similar_images(
            unprocessed_images, sensitivity, threads
        )
    ...
    if unprocessed_videos:
        similar_video_groups = find_similar_videos(
            unprocessed_videos, sensitivity, threads
        )
