...
**SnapTidy**는 복잡한 디렉토리를 정리하고 중복 파일을 제거하는 강력한 도구입니다 - 특히 사진과 기타 미디어 파일에 특화되어 있습니다. 같은 사진을 여러 번 다운로드하거나 수십 개의 폴더에 분산된 사진들로 인해 고민이신가요? SnapTidy로 간편하게 정리하세요.

## 🚀 빠른 시작

### 대부분의 사용자: GUI 인터페이스 (권장)

SnapTidy는 사진 정리를 간편하게 만들어주는 아름답고 직관적인 GUI 애플리케이션을 포함합니다!
...
고급 사용자는 자동화와 스크립팅을 위해 CLI를 사용할 수 있습니다:
...
### 실행 및 설정
1. **SnapTidy 설치** (아래 설치 섹션 참조)
2. **GUI 실행**: 터미널에서 `snaptidy-gui` 실행
3. **폴더 선택**: 폴더 선택기 사용 또는 드래그앤드롭
4. **설정 구성**: 직관적인 컨트롤로 옵션 조정
5. **작업 실행**: 개별 작업 버튼 또는 "모두 실행" 클릭
